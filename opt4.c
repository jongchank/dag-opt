...
static double jconv4(double p1, double p2, double p3, double p4);
...
int main(void)
{
    ...
    for (p1 = MIN_P; p1 <= MAX_P; p1++) {
        if (jconv4(p1, 1, 1, 1)  > min_j)
            break;
        if (util1(e1, p1) > 1)
            continue;
        for (p2 = MIN_P; p2 <= MAX_P; p2++) {
            if (jconv4(p1, p2, 1, 1) > min_j)
                break;
            if (util2(e1, p1, e2, p2) > 1)
                continue;
            for (p3 = MIN_P; p3 <= MAX_P; p3++) {
                if (jconv4(p1, p2, p3, 1) > min_j)
                    break;
    ...
    printf("J = %g\n", jconv4(opt_p1, opt_p2, opt_p3, opt_p4) / 2);
    ...
    printf("J = %g\n", jconv4(real_p1, real_p2, real_p3, real_p4) / 2);
